...
which optimizes one second-order low-shelf and one first-order high-shelf per
0.5 phon step (35–95 phon). Firmware embeds the results as `lowshelf_and_volume_*`
(shape + baked playback gain), `lowshelf_no_volume_*` (shape only), and
`highshelf_no_volume_*` (treble contour only). See [LOUDNESS.md](LOUDNESS.md) for
how each filter mode selects tables and volume routing.
