...
static biquad_quotients_fast_t
    hires_halfrate_runtime_slot[2][LOUDNESS_CHANNELS];
static volatile uint8_t hires_active_quotient_slot;
static uint32_t loudness_highres_frequency_hz;

static loudness_highres_channel_state_t loudness_highres_ch[2];
static loudness_highres_lowshelf_step_stride_fn loudness_highres_lowshelf_stride_step =
    NULL;

static biquad_quotients_fast_t loudness_highres_quotients_scaled[
    LOUDNESS_NUM_EQUALIZER_STEPS];

static inline void loudness_highres_memory_barrier(void)
{
#if defined(__GNUC__)
...
#define LOUDNESS_HIGHRES_DEFINE_BIQUAD1_STEP_STRIDE(fn_name, shift_plus_one, counter_mask) \
int32_t fn_name(int32_t x_24, biquad_state_fast_t *st,                                      \
    loudness_highres_channel_state_t *ch, const biquad_quotients_fast_t *q)                 \
{                                                                                             \
    if (ch->sample_counter == 0) {                                                            \
        int32_t y_true = loudness_lowshelf(x_24, st, q);                         \
...
LOUDNESS_HIGHRES_DEFINE_BIQUAD1_STEP_STRIDE(
    loudness_highres_lowshelf_step_runtime_stride4, 3, 0x03)
LOUDNESS_HIGHRES_DEFINE_BIQUAD1_STEP_STRIDE(
    loudness_highres_lowshelf_step_runtime_stride2, 2, 0x01)
...
static void loudness_highres_fill_inactive_quotients_fast(
    int equalizer_step_left, int equalizer_step_right)
{
    ...
    biquad_quotients_fast_t main_quotients[LOUDNESS_CHANNELS];
    ...
    hires_src_right = loudness_highres_halfrate_quotients(
        ...
        loudness_fast_baked_quotient_table_44100hz(),
        loudness_fast_baked_quotient_table_48000hz());
    loudness_fast_prepare_inactive_quotients(table, equalizer_step_left,
        equalizer_step_right);
    main_quotients[0] = *loudness_lowshelf_quotients(0);
    main_quotients[1] = *loudness_lowshelf_quotients(1);
    loudness_highres_fill_inactive_halfrate_quotients(
        hires_src_left, hires_src_right, main_quotients);
}
...
void loudness_highres_change_frequency(uint32_t frequency)
{
    ...
    loudness_highres_fill_inactive_quotients_fast(equalizer_step_left,
        equalizer_step_right);
    loudness_fast_publish_quotients();
    loudness_highres_publish_halfrate_quotients();
...
void loudness_highres_set_stride(uint32_t sample_rate_hz)
{
    ...
    loudness_highres_lowshelf_stride_step = (sample_rate_hz >= (uint32_t)FREQ_176)
        ? loudness_highres_lowshelf_step_runtime_stride4
        : loudness_highres_lowshelf_step_runtime_stride2;
}
...
{
    uint8_t inactive = (uint8_t)(hires_active_quotient_slot ^ 1u);
    int channel;
    ...
    for (channel = 0; channel < LOUDNESS_CHANNELS; channel++) {
        ...
        if (src != NULL) {
            hires_halfrate_runtime_slot[inactive][channel] = *src;
        } else {
            hires_halfrate_runtime_slot[inactive][channel] = main_quotients[channel];
        }
    }
}
...
{
    loudness_highres_memory_barrier();
    hires_active_quotient_slot ^= 1u;
}
 ...
 */
static void loudness_highres_filter_16bit_stereo_packet_with_quotients(
    S32 *sample_L, S32 *sample_R,
    biquad_state_fast_t *stL, biquad_state_fast_t *stR,
    const biquad_quotients_fast_t *quotients_left,
    const biquad_quotients_fast_t *quotients_right, U16 num_samples)
{
    loudness_highres_lowshelf_step_stride_fn step = loudness_highres_lowshelf_stride_step;
    Bool idle_L = loudness_channel_filter_is_idle(0);
    int i;
    ...
    for (i = 0; i < num_samples; i++) {
        ...
        if (sample_L[i] == 0 && idle_L) {
            ...
            yL = step(xL, stL, &loudness_highres_ch[0], quotients_left);
        ...
        if (sample_R[i] == 0 && loudness_channel_filter_is_idle(1)) {
            ...
            yR = step(xR, stR, &loudness_highres_ch[1], quotients_right);
        }
...
{
    uint8_t slot = hires_active_quotient_slot & 1u;
    const biquad_quotients_fast_t *quotients = hires_halfrate_runtime_slot[slot];

    loudness_highres_filter_16bit_stereo_packet_with_quotients(
        sample_L, sample_R, stL, stR, &quotients[0], &quotients[1], num_samples);
}
    ...
    S32 *sample_L, S32 *sample_R, U16 num_samples)
{
    ...
    loudness_highres_filter_16bit_stereo_packet_independent(sample_L, sample_R, stL, stR, num_samples);
    loudness_fast_refresh_idle_cache();
}
...
void loudness_highres_test_load_active_quotients(int equalizer_step)
{
    ...
    loudness_fast_set_active_equalizer_step_table(table);
    loudness_highres_fill_inactive_quotients_fast(equalizer_step, equalizer_step);
    loudness_fast_publish_quotients();
    loudness_highres_publish_halfrate_quotients();
}
    ...
    const biquad_quotients_fast_t *quotients, U16 num_samples)
{
    ...
    for (i = 0; i < num_samples; i++) {
        int32_t xL = loudness_highres_s16_to_24bit(sample_L[i]);
        int32_t xR = loudness_highres_s16_to_24bit(sample_R[i]);
        int32_t yL = loudness_lowshelf(xL, stL, &quotients[0]);
        int32_t yR = loudness_lowshelf(xR, stR, &quotients[1]);
