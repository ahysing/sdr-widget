...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Overlays a fixed 3-biquad (low-shelf + peak + high-shelf) chain on ISO 226 curves."
    )
...
# Initial Fixed biquad chain specification: (name, fc, Q, gain_db, kind)
BIQUAD_CHAIN_INIT = [
    ("low-shelf",  120.0,  0.5,   12.0, "low_shelf"),   # Positive bass boost guess
    ("peak",       3500.0, 1.2,   -3.5, "peak"),        # Midrange dip correction guess
...
def biquad_peaking(fc, Q, gain_db, sample_r):
    A = 10**(gain_db/40)
    w0 = 2*np.pi*fc/sample_r
    alpha = np.sin(w0)/(2*Q)
...
def chain_response(params, frequencies_hz, sample_rate_hz):
    fc1, Q1, g1, fc2, Q2, g2, fc3, Q3, g3 = params
    ...
    H *= freq_response(biquad_peaking(fc2, Q2, g2, sample_rate_hz), frequencies_hz, sample_rate_hz)
    H *= freq_response(biquad_high_shelf(fc3, Q3, g3, sample_rate_hz), frequencies_hz, sample_rate_hz)
...
def plot(args, optimized_params_per_phon, f_eval, sample_rate_hz):
    ...
    plt.title(f"Graph type: {args.graph_type} (fs={sample_rate_hz}Hz)")
    plt.tight_layout()
    plt.show()
    ...
    if args.frequencymode == "both":
        ...
        for name, hz in SAMPLE_RATES_HZ:
            if name == args.frequencymode:
                rates_to_process = [hz]
                break
    section_names = ["low-shelf", "peak", "high-shelf"]
    for sample_rate_hz in rates_to_process:
        ...
        for phon in PHON_LEVELS:
            ...
            init = [
                ...
                BIQUAD_CHAIN_INIT[1][1], BIQUAD_CHAIN_INIT[1][2], BIQUAD_CHAIN_INIT[1][3], # peak
                BIQUAD_CHAIN_INIT[2][1], BIQUAD_CHAIN_INIT[2][2], BIQUAD_CHAIN_INIT[2][3], # high-shelf
            ]

            # Bounds (Fix: Constrain the peaking filter to the midrange region)
            lower = [
                20,   0.1, -40,  # low-shelf (fc min, Q min, gain min)
                2500, 0.4, -20,  # peak (Shifted fc min from 500 to 2500, Q min to 0.4)
                4000, 0.1, -40   # high-shelf
            ]
            upper = [
                300,   2.0, 40,  # low-shelf (Capped fc max at 300 Hz instead of 2000 Hz)
                6000,  4.0, 20,  # peak (Capped fc max at 6000 Hz, restricted gain range)
        ...
        for phon in PHON_LEVELS:
            ...
            for i, name in enumerate(section_names):
                ...
                if name == "low-shelf":
                    ...
                    coeffs = biquad_peaking(fc, Q, dsp_gain, sample_rate_hz)
